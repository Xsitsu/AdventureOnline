...
void PrintEmployees(char driver[])
{
	...
	SQLHANDLE hstmt;
	//SDWORD cbQual;
	char* query = "select * from tablewise.NPC";
	char ename[100];
	SQLRETURN retcode;
	...
	sprintf((char *)InConnStr, "driver={ODBC Driver 13 for SQL Server}; Server=aura.students.cset.oit.edu;Database=jose_morenodedios;uid=jose_morenodedios;pwd=shaedar2123", driver);
	cout << InConnStr << endl;
				...
				if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
				{
					...
					while (!done)
					{
						...
						if (fetch_rc == SQL_SUCCESS || fetch_rc == SQL_SUCCESS_WITH_INFO)
						{
							...
							SQLGetData(hstmt, 2, SQL_C_CHAR, ename, 100, &cbEname);
							cout << ename << '\t' ;
							SQLGetData(hstmt, 3, SQL_C_CHAR, ename, 100, &cbEname);
							cout << ename << endl;
						}
						else
							done = true;
